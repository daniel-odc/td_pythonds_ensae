...
![](./img/git_fichiers_multiples.png)    
...
* `Git` n’est pas `Github` ou `Gitlab`
...
| `git pull origin main`      | Récupération de la dernière version du code depuis le repo remote (opération `« fetch »`), et fusion avec changements locaux si nécessaire (opération `« merge »`) (`git pull origin master` au lieu de `main` dans des versions anciennes de projets `git`)|
...
| `git push origin main`      | Envoi des dernières modifications locales sur le repo remote (`git push origin master` au lieu de `main` dans des versions anciennes de projet `git`)                                                                                                       |
...
> Puis ajouter les collaborateurs en utilisant leur identifiant `GitHub` (visible sur le profile)

:::

::: column
![](./img/git_collaborators.jpg)
:::
:::
